...
using System.Data; // For CopyToDataTable
...
namespace System.Linq.Dynamic
{
    ...
    public static class DynamicQueryable
    {
        ...
        /*
        public static bool IsSortExpression(string strExpression)
        {
            if (StringComparer.OrdinalIgnoreCase.Equals(strExpression, "ASC") || StringComparer.OrdinalIgnoreCase.Equals(strExpression, "DESC"))
                return true;
        ...
        */
        ...
        public static string GetColumnExpression(IQueryable source, string strColumnName)
        {
            int iOrdinal = 0;

            foreach (System.Data.DataRow row in source)
            {
                iOrdinal = row.Table.Columns[strColumnName].Ordinal;
                break;
            }

            string strFieldName = "ItemArray[" + iOrdinal.ToString() + "]"; // " " + astrParts[i];
            return strFieldName;
        }
        ...
        public static IQueryable OrderBy(this IQueryable source, string ordering, params object[] values)
        {
            ...
            {
                // TODO: DbNull doesn't implement IComparable ==> Exception
                using (System.Data.DataTable dt = source.Cast<System.Data.DataRow>().CopyToDataTable())
                {
                    return dt.Select("", ordering).AsQueryable();
                } // End using dt

                string[] arrExpressions = ordering.Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries);

                for (int i = 0; i < arrExpressions.Length; ++i)
                {
                    string[] arrParts = arrExpressions[i].Split(new char[] { ' ', '\t' }, StringSplitOptions.RemoveEmptyEntries);

                    if (arrParts.Length > 0)
                    {
                        arrParts[0] = GetColumnExpression(source, arrParts[0]);

                        if (arrParts.Length > 1)
                        {
                            arrExpressions[i] = arrParts[0] + " " + arrParts[1];
                        }
                        else
                            arrExpressions[i] = arrParts[0];
                    } // End if (arrParts.Length > 0)

                    Array.Clear(arrParts, 0, arrParts.Length);
                    arrParts = null;
                } // Next i

                ordering = string.Join(", ", arrExpressions);
                Array.Clear(arrExpressions, 0, arrExpressions.Length);
                arrExpressions = null;

                /*
                //string[] arrParts = ordering.Split(new char[] { ' ', '\t', ',' }, StringSplitOptions.RemoveEmptyEntries);
                //System.Collections.Generic.Queue<string> qSortExpressions = new Collections.Generic.Queue<string>();
                System.Collections.Generic.LinkedList<string> llSortExpressions = new Collections.Generic.LinkedList<string>();


                foreach (string strExpression in arrParts)
                {
                    //qSortExpressions.Enqueue(strExpression);
                    llSortExpressions.AddLast(strExpression);
                }


                string strSortExpression = null;

                while (llSortExpressions.Count > 0)
                {
                    string strThisValue = llSortExpressions.First.Value;
                    llSortExpressions.RemoveFirst();

                    string strNextValue = null;

                    if (llSortExpressions.First != null)
                        strNextValue = llSortExpressions.First.Value;

                    string strThisSortExpression = null;


                    if (IsSortExpression(strNextValue))
                    {
                        string strColumn = GetColumnExpression(source, strThisValue);
                        strThisSortExpression = strColumn + " " + strNextValue;
                        llSortExpressions.RemoveFirst();
                    }
                    else
                    {
                        string strColumn = GetColumnExpression(source, strThisValue);
                        strThisSortExpression = strColumn;
                    }

                    if (strSortExpression != null)
                        strSortExpression += ", " + strThisSortExpression;
                    else
                        strSortExpression = strThisSortExpression;
                }

                ordering = strSortExpression;
                */

                /*
                string[] astrParts = ordering.Split(' ');
                
                int iOrdinal = 0;
                foreach (System.Data.DataRow row in source)
                {
                    iOrdinal = row.Table.Columns[astrParts[0]].Ordinal;
                    break;
                }

                ordering = "ItemArray[" + iOrdinal + "]";
                for (int i = 1; i < astrParts.Length; ++i)
                {
                    ordering += " " + astrParts[i];
                }

                astrParts = null;
                */
            } // End if (object.ReferenceEquals(source.ElementType, typeof(System.Data.DataRow)))
